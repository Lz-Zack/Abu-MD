...
┃☼︎╭──────────────
┃☼︎│
┃☼︎│ plugins : ${commands.length.toString()}
┃☼︎│ user :- @${m.number}
┃☼︎│ owner : ${(info_vars[1] || '').replace(/[;,|]/g,'')}
┃☼︎│ version: ${package.version}
┃☼︎│ prefix:- ${prefix}
┃☼︎│ mode :- ${MODE}
┃☼︎│ date :- ${date}
┃☼︎│ ram :- ${format(os.totalmem()-os.freemem())}
┃☼︎│ 
┃☼︎╰───────────────
╰══════════════════⊷❍`;
	...
	category.forEach((cmmd) => {
		menu += `
╭═══〘 ${addSpace("*"+cmmd.toUpperCase()+"*",10,"both")} 〙═══⊷❍
┃☼︎╭───────────❍`;
