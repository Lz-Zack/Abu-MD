...
const isPublic = (config.MODE && config.MODE.toLowerCase().trim()) == 'public' ? 'public' : 'private';
...
const WhatsBotConnect = async () => {
	...
	try {
		...
		conn.ev.on("connection.update", async ({
			...
			if (connection == "connecting") console.log("Session Restored..✅!");
				...
				conn.ev.on("messages.upsert", async (chatUpdate) => {
					...
					commands.map(async (command) => {
						...
						if (isPublic == 'private' && !m.isCreator && command.fromMe) em_ed = "active";
						if (isPublic == 'public' && command.fromMe == true && !m.isCreator) em_ed = "active";
		...
		cron.schedule('*/30 * * * *', async () => {
			...
			try {
				...
				if (commit_key != Commit_key && Commit_key != "jsl") {
					...
					const update_msg = "_There have some updates_";
					let description = "";
